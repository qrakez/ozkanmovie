<?php
session_start();

include 'settings/db.php'; 
include 'func.php';

$q = isset($_GET['q']) ? $_GET['q'] : '';

?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>OzkanMovie | Kategoriler</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
    <link rel="stylesheet" href="styles/style.css">
</head>
<body class="bg-dark">
    <div class="container-fluid">
        <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
            <div class="container-fluid">
                <a class="navbar-brand" href="#">OZKANMOVIE</a>
                <button class="navbar-toggler" type="button" data-bs-toggle="collapse"
                    data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent"
                    aria-expanded="false" aria-label="Toggle navigation">
                    <span class="navbar-toggler-icon"></span>
                </button>
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                        <li class="nav-item">
                            <a class="nav-link active" aria-current="page" href="index.php">Ana Sayfa</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="movie.php">Filmler</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="user-favorite-movie.php">Favori Filmlerim</a>
                        </li>
                    </ul>
                    <ul class="navbar-nav mb-2 mb-lg-0 ms-auto">
                        <li class="nav-item">   
                            <a class="nav-link" href="#"><i class="bi bi-person-circle"></i> <?=@$_SESSION['user']?></a>
                        </li>
                        <li class="nav-item">   
                            <a class="nav-link" href="logout.php"><i class="bi bi-person-fill-dash"></i> Çıkış Yap</a>
                        </li>
                    </ul>
                </div>
            </div>
        </nav>
        <div class="action-categorie">
            <div class="row ms-2">
                <?php
                if (!empty($q)) {
                    $categoryQuery = $db->prepare("SELECT * FROM categories WHERE self_link = ?");
                    $categoryQuery->execute([$q]);
                    $category = $categoryQuery->fetch(PDO::FETCH_ASSOC);
                
                    if ($category) {
                        $categoryId = $category['id'];
                        echo ' <h4 style="border-bottom: 2px solid #fff; padding: 10px; color: #fff;">'.$category["name"].'</h4>';
                        $moviesQuery = $db->prepare("SELECT * FROM movies WHERE category_id = ?");
                        $moviesQuery->execute([$categoryId]);
                        $movies = $moviesQuery->fetchAll(PDO::FETCH_ASSOC);
                
                        foreach ($movies as $movie) {
                            echo '
                            <div class="col-md-6 col-lg-4 text-white">
                                <div class="series-item" style="height: 475px;">
                                    <img src="' . $movie["banner_url"] . '" alt="' . $movie["title"] . '" class="img-fluid d-block mx-auto">
                                    <div class="series-detail">
                                        <p class="card-series-header">' . $movie["title"] . '</p>
                                        <p class="card-series-detail">' . $movie["description"] . '</p>
                                        <a href="watch-movie.php?q=' . $movie["self_link"] . '" class="card-watch-btn">İzle</a>
                                    </div>
                                </div>
                            </div>
                            ';
                        }
                    } else {
                        echo "Kategori bulunamadı";
                    }
                } else {
                    echo "Kategori belirtilmedi";
                }
                ?>
</div>         
    </div>
</body>
</html>